...
void* rcv(int sockfd){//不能只接收服务器转发的消息，在这里就接收login后服务器发送的所有消息！
	int nbytes;
	...
	while(FLAG==0){
		...
		if(!strncmp(buf,"\n##FFI",6)){
			...
			close(fd);
			printf("file over\n");
			printf("2:Query the online list;3:Chat;4:delete;5:logout;6:send file\n");
		}
	}
	pthread_exit (NULL);
}
...
int main(int argc,char **argv)
{
	...
	if(!strcmp(temp,"0"))
	{
		...
		//<应该写在服务器的>成功：You have registered successfully!失败：1、wrong:格式错误；2、wrong：用户名已存在
		//比对接受来的前5个字符是不是wrong
		//传过来的直接输出
	...
	while(1){
		...
		if(!strcmp(temp,"2")){
			...
			printf("please input your 'password'\n");//(2)这里只输入密码确认一下就可以了，传到服务器：username#password
			gets(cmd);
			bzero(buf,sizeof(buf));
			...
			//嗯，失败的话字符串长度是15，如果传回来的字符串>15，就是删除成功了
			/*
			nbytes=read(sockfd,buf,MAXDATASIZE);
			buf[nbytes]='\0';
			printf("recv message from server:%s\n",buf);
		...
		else if(!strcmp(temp,"5")){//把name传回去，直接exit吧
			flag0=1;
			//name传回去
			sleep(1);
			bzero(buf,sizeof(buf));
